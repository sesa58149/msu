...
#ifndef _MSU_COMPLAIN_C__
#define _MSU_COMPLAIN_C__
...
BOOL setRecvdChnkBit(UINT32 chnkNum)
{
 ...
 if( (MIN_NUM_CHNKS > chnkNum) || (chnkNum > MAX_NUM_CHUNKS_SUPPORTED) )
 {
#ifdef	Debug_level1
	 printf("\nOut of proper chunk range\n");
#endif
	 return 0; /*avoid array over run*/
 }
...
BOOL isChnkRecvd(UINT32 chnkNum)
{
   ...
   BOOL retVal = 0;
 	...
 	index = (chnkNum -1)/CHUNK_NUM_DIV;
 	mask = 1 << ((chnkNum-1)%CHUNK_NUM_DIV);
