...
#ifndef MSUPLATFORMHPP
#define MSUPLATFORMHPP
...
#define MSU_PLATFORM_BIG_ENDIAN
...
#define ETHERBRICK_PLATFORM 1

#define CRC_VALIDATION_REQUIRED1
#define CHUNK_IN_RAM
//#define MSU_MODBUS_IDENTIFICATION
//#define RETRY
//#define DEBUG_LEVEL0
//#define DEBUG_LEVEL1
//#define DEBUG_LEVEL2
...
/*Byte ordering macros*/
	...
	#define trHtoNs(x)      trHtons(x)
	#define trNtoHs(x)      trNtohs(x)
	#define trHtoNl(x)      trHtonl(x)
	#define trNtoHl(x)      trNtohl(x)
...
#define MsuBasicIdentification(structure)     BasicIdentification(structure)
#ifdef ETHERBRICK_PLATFORM
	#define msuLogMsg(x,y,z)LogMsg(x,y,z)
#else
	#define msuLogMsg(x,y,z)printf(y,z)
#endif
